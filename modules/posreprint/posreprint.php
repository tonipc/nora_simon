...
class posreprint extends Module
{
	// public $output = '';

    public function __construct()
    {
        $this->name = 'posreprint';
    ...
    // {
    //     $this->context->controller->registerJavascript('module-'.$this->name.'-js', 'modules/'.$this->name.'/views/js/reprint.js', ['position' => 'bottom', 'priority' => 100]);
    ...
    public function hookActionPosSetMedia($params)
    {
        $this->context->controller->posAddCss(
            [
                _MODULE_DIR_ . $this->name . '/views/css/reprint.css',
            ]
        );
        $this->context->controller->posAddJs(
            [
                _MODULE_DIR_ . $this->name . '/views/js/reprint.js',
            ]
        );
    }
    ...
    public function hookDisplayWkPosReprint($params)
    {
        ...
        return $this->display(__FILE__, 'reprint.tpl');
